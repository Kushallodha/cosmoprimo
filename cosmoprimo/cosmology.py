...
def _compute_ncdm_momenta(T_eff, m, z, method='laguerre', epsabs=1e-7, epsrel=1e-7, out='rho'):
    ...
    if method == 'quad':
        ...
        if out == 'rho':
            ...
            raise ValueError('Cannot compute ncdm momenta {}; \
                             choices are ["rho", "drhodm", "p"]', out)
    ...
    def get(self, name, default_value=None):
        ...
        name : string
            Name of the parameter. One of following parameters:
                ...
                'm_ncdm_tot' : total mass of non-cold dark matter species,
                'Omega_ncdm' : non-cold dark matter density,
                'Omega_ncdm_tot' : total non-cold dark matter density,
                'Omega_pncdm' : non-cold dark matter pressure density,
                'Omega_pncdm_tot' : total non-cold dark matter pressure density,
                'Omega_m' : matter density,
                'Omega_de' : dark energy density,
                'Omega_Lambda' : cosmological constant density,
                'Omega_fld' : fluid dark energy density,
        ...
        default_value : float, optional, default=None
            Default value of a given parameter if it is not found.
        ...
        try:
            ...
            if name == 'Omega_g':
                rho = params['T_cmb']**4 * 4. / constants.c**3 *\
                    constants.Stefan_Boltzmann  # density, kg/m^3
            ...
            if name == 'Omega_ur':
                rho = params['N_ur'] * 7. / 8. * self.get('T_ur')**4 * 4. /\
                    constants.c**3 * constants.Stefan_Boltzmann  # density, kg/m^3
                return rho / (self.get('h')**2 * constants.rho_crit_over_kgph_per_mph3)
            if name == 'Omega_r':
                rho = (params['T_cmb']**4 + params['N_ur'] * 7. / 8. * self.get('T_ur')**4) * 4. /\
                      constants.c**3 * constants.Stefan_Boltzmann
                return rho / (self.get('h')**2 * constants.rho_crit_over_kgph_per_mph3) +\
                    self.get('Omega_pncdm_tot')
            if name == 'm_ncdm_tot':
                return sum(params['m_ncdm'])
            if name == 'Omega_ncdm':
                derived['Omega_ncdm'] = self._get_ncdm(z=0, out='rho') /\
                    constants.rho_crit_over_Msunph_per_Mpcph3
                return derived['Omega_ncdm']
            if name == 'Omega_ncdm_tot':
                return sum(self.get('Omega_ncdm'))
            if name == 'Omega_pncdm':
                derived['Omega_pncdm'] = 3. * self._get_ncdm(z=0, out='p') /\
                    constants.rho_crit_over_Msunph_per_Mpcph3
                return derived['Omega_pncdm']
            if name == 'Omega_pncdm_tot':
                return sum(self.get('Omega_pncdm'))
            if name == 'Omega_m':
                return self.get('Omega_b') + self.get('Omega_cdm') +\
                    self.get('Omega_ncdm_tot') - self.get('Omega_pncdm_tot')
            if name == 'Omega_de':
                return 1. - sum(self.get(name) for name in
                    ['Omega_cdm', 'Omega_b', 'Omega_g', 'Omega_ur', 'Omega_ncdm_tot', 'Omega_k'])
            ...
            if name == 'theta_cosmomc':
                ...
                rs, zstar = _compute_rs_cosmomc(
                    self['omega_b'], self['omega_m'], ba.hubble_function
                    )
...
class DefaultBackground(BaseBackground):
    ...
    def time(self, z):
        ...
        if name not in self._cache:
            ...
            self._cache[name] = Interpolator1D(
                zc, (tmp[-1] - tmp) / self.h / constants.gigayear_over_megaparsec
                )
    ...
    def comoving_radial_distance(self, z):
        ...
        if name not in self._cache:
            ...
            zc = get_default_z_interp(name)
            tmp = odeint(integrand, 0., zc)
            # cubic interpolation takes a lot of time, but is very efficient
